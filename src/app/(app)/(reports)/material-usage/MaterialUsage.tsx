...
import { useQueryGetAllMaterialUsage } from "./_api/queries/useQueryGetAllMaterialUsage";
import { useQueryDownloadMaterialUsage } from "./_api/queries/useQueryDownloadMaterialUsage";
...
export default function MaterialUsage() {
  ...
  const { data: materialUsageData, status } = useQueryGetAllMaterialUsage({
    // enabled,
  });
  console.log("👉 ~ MaterialUsage ~ materialUsageData:", materialUsageData);
  const {
    data: downloadData,
  ...
  useEffect(() => {
    if (
      ...
      console.log("✅ ~ MaterialUsage ~ downloadData:", downloadData);
  ...
  console.log("👉 ~ MaterialUsage ~ data:", data);
  type MaterialUsageAPIType = (typeof data.data)[number];

  const columns: ColumnDef<MaterialUsageAPIType>[] = (table_headers ?? []).map(
    (data: any) => ({
      header: data.header,
      accessorKey: data.accessor_key,
      cell: (props: any) => {
        const name = props.getValue() as string;
        console.log("✅ ~ MaterialUsage ~ name:", name);
        return name;
      },
    })
  );
