...
import { useQueryGetAllPurchaseSummary } from "./_api/queries/useQueryGetAllPurchaseSummary";
import { useQueryDownloadPurchaseSummary } from "./_api/queries/useQueryDownloadPurchaseSummary";
...
export default function PurchaseSummary() {
  ...
  const { data: purchaseSummaryData, status } = useQueryGetAllPurchaseSummary({
    // enabled,
  });
  console.log(
  ...
  const {
    ...
    isFetching,
    isSuccess,
    refetch: downloadPurchaseSummary,
  } = useQueryDownloadPurchaseSummary({
    // enabled,
  });
  useEffect(() => {
    if (
      ...
      console.log("✅ ~ PurchaseSummary ~ downloadData:", downloadData);
  ...
  type PurchaseSummaryAPIType = (typeof data.data)[number];

  const columns: ColumnDef<PurchaseSummaryAPIType>[] = (
    table_headers ?? []
  ).map((data: any) => ({
    header: data.header,
    ...
    cell: (props: any) => {
      ...
      console.log("✅ ~ PurchaseSummary ~ name:", name);
      return name;
    },
  }));
