...
import { useQueryGetAllReportInventoryStatus } from "./_api/queries/useQueryGetAllReportInventoryStatus";
...
import { useQueryDownloadReportInventoryStatus } from "./_api/queries/useQueryDownloadReportInventoryStatus";
...
export default function ReportInventoryStatus() {
  ...
  const { data, status } = useQueryGetAllReportInventoryStatus({
    // enabled,
  });
  const {
    data: downloadData,
  ...
  } = useQueryDownloadReportInventoryStatus({
    // enabled,
  });
  useEffect(() => {
    if (
  ...
  const { table_headers, data: tableData } = data ?? {};
  console.log("✅ ~ ReportInventoryStatus ~ tableData:", tableData?.data);
  type ReportInventoryStatusAPIType = (typeof data.data)[number];
  {
    /*
  headings:[
  {
  header:"Branch",
  accessor_key:"branch_name",
  },...
  ]
  */
  }
  const columns: ColumnDef<ReportInventoryStatusAPIType>[] = (
    table_headers ?? []
  ).map((data: any) => ({
    header: data.header,
    ...
    cell: (props: any) => {
      ...
      console.log("✅ ~ ReportInventoryStatus ~ name:", name);
      return name;
    },
  }));
  ...
  return (
    <div>
      ...
      <AppStatus status={status} is_data={!!tableData?.data?.length}>
        <AppTable data={tableData?.data ?? []} columns={columns} />
        <AppPagination
